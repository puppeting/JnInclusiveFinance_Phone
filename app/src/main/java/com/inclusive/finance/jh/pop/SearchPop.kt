...
import com.inclusive.finance.jh.databinding.PopSearchRecyclerBinding
import razerdp.basepopup.BasePopupWindow
import razerdp.util.animation.AlphaConfig
import razerdp.util.animation.AnimationHelper


class SearchPop : BasePopupWindow, OnItemClickListener {
    ...
    var listener: ((jgcode: String, jyzt: String, lczt: String, sfdb: String,khmc: String,enums12: LinkedHashMap<String, ArrayList<BaseTypeBean.Enum12>>?) -> Unit)? = null

    constructor(context: Dialog?, enums12: LinkedHashMap<String, ArrayList<BaseTypeBean.Enum12>>?, checkedTextView: View?, isSingleChecked: Boolean = true, listener: ((jgcode: String, jyzt: String, lczt: String, sfdb: String,khmc: String,enums12: LinkedHashMap<String, ArrayList<BaseTypeBean.Enum12>>?) -> Unit)? = null) : super(context) {
        init(bean, enums12, checkedTextView, isSingleChecked, listener)
    }

    constructor(context: Context?, bean: BaseTypeBean? = null, enums12: LinkedHashMap<String, ArrayList<BaseTypeBean.Enum12>>?, checkedTextView: View?, isSingleChecked: Boolean = true, listener: ((jgcode: String, jyzt: String, lczt: String, sfdb: String,khmc: String,enums12: LinkedHashMap<String, ArrayList<BaseTypeBean.Enum12>>?) -> Unit)? = null) : super(context) {
        init(bean, enums12, checkedTextView, isSingleChecked, listener)
    }

    private fun init(bean: BaseTypeBean?, enums12: LinkedHashMap<String, ArrayList<BaseTypeBean.Enum12>>?, checkedTextView: View?, isSingleChecked: Boolean, listener: ((jgcode: String, jyzt: String, lczt: String, sfdb: String,khmc: String,enums12: LinkedHashMap<String, ArrayList<BaseTypeBean.Enum12>>?) -> Unit)?) {
        this.bean = bean
        ...
        expandableListView!!.setOnChildClickListener { parent, v, groupPosition, childPosition, id ->
            ...
            enums12?.get((titleList as ArrayList<String>)[groupPosition])?.forEachIndexed { index, enum12 ->
            ...
            enums12?.let { (adapter2 as CustomExpandableListAdapter?)?.setPoss(it) }
             when ((titleList as ArrayList<String>)[groupPosition]) {
                "机构" -> {jgcode = enums12?.get((titleList as ArrayList<String>)[groupPosition])
...
lateinit var dataBind: PopSearchRecyclerBinding
override fun onCreateContentView(): View { //        dataBind = DataBindingUtil.inflate(LayoutInflater.from(context),R.layout.pop_down_recycler,null,false)
    dataBind = PopSearchRecyclerBinding.bind(createPopupById(R.layout.pop_search_recycler))
...
override fun onDismiss() {
    if (isChecked) listener?.invoke(jgcode,jycode,lccode,dbcode, dataBind?.tvValue?.text.toString(),enums12)
