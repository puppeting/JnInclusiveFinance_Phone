...
import com.inclusive.finance.jh.databinding.PopLishiBinding
import com.inclusive.finance.jh.utils.SZWUtils
import com.inclusive.finance.jh.utils.StatusBarUtil

class LinShiPop(var mContext: Context?, val json: JsonObject? = null, var businessType: Int? = 0, var listener: (String) -> Unit? = {}) :
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.tv_ok -> {
                if (dataBind.ghr?.text.toString() == "") {
                    SZWUtils.showSnakeBarError(dataBind.root, "请选择合同类型")
                    return
                }
                listener.invoke(dataBind.ghr?.text.toString())
                dismiss()
//                when (businessType) {
//                    ApplyModel.BUSINESS_TYPE_HTQY -> {////授信申请-征信管理-提交-新增选择审批人（新接口）
//                        if (dataBind.ghr?.text.toString() == "") {
//                            SZWUtils.showSnakeBarError(dataBind.root, "请选择合同类型")
//                            return
//                        }
...
//                }
    ...
    lateinit var dataBind: PopLishiBinding
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View { //        dialog?.requestWindowFeature(Window.FEATURE_NO_TITLE)
        ...
        dataBind = PopLishiBinding.inflate(inflater, container, false)
...
//                        if (position != -1&&businessType!= ApplyModel.BUSINESS_TYPE_APPLY) {//授信申请-征信管理-提交-新增选择审批人（新接口）
                            DataCtrlClass.JNJNet.get_hetong(context, ghrUrl) { ghrList ->
                                if (ghrList != null) {
...
//        val ghrDownList = arrayListOf<BaseTypeBean.Enum12>()
//        var bean=BaseTypeBean.Enum12().apply {
//            keyName = "个人信用借款合同"
//            valueName = "个人信用借款合同"
//        }
//        ghrDownList.add(bean)
//        dataBind.ghr.setOnClickListener { view ->
//            DownPop(dialog, enums12 = ghrDownList, checkedTextView = view as AppCompatCheckedTextView, isSingleChecked = businessType != ApplyModel.BUSINESS_TYPE_QRCODE) { key, value, _ ->
//                dataBind.ghr?.text=value
//            }.showPopupWindow(view)
