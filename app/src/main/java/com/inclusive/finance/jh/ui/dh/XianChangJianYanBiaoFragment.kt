...
import com.inclusive.finance.jh.databinding.FragmentXianchangjianyanbiaoBinding
 ...
 * 现场检验表
 * */
class XianChangJianYanBiaoFragment : MyBaseFragment(), PresenterClick {
    lateinit var adapter: ItemBaseTypeAdapter<BaseTypeBean>
    lateinit var viewModel: ApplyModel
    lateinit var viewBind: FragmentXianchangjianyanbiaoBinding
    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?,
    ): View {
        viewBind = FragmentXianchangjianyanbiaoBinding.inflate(inflater, container, false).apply {
            presenterClick = this@XianChangJianYanBiaoFragment
    ...
    override fun initView() {
        adapter = ItemBaseTypeAdapter(this@XianChangJianYanBiaoFragment)
...
//                adapter.setNewInstance(list)
        when (viewModel.businessType) {
            ApplyModel.BUSINESS_TYPE_JNJ_JC_ON_SITE_PERSONAL -> {
                getUrl=Urls.get_jnj_jc_personal_xcjyb
                saveUrl=Urls.save_jnj_jc_personal_xcjyb
//                businessType="03"
            }
            ApplyModel.BUSINESS_TYPE_JNJ_JC_ON_SITE_COMPANY -> {
                getUrl=Urls.get_jnj_jc_company_xcjyb
                saveUrl=Urls.save_jnj_jc_company_xcjyb
//                businessType="03"
            }
        }
