...
import com.inclusive.finance.jh.databinding.PopQueryRecyclerBinding
import razerdp.basepopup.BasePopupWindow
import razerdp.util.animation.AlphaConfig
import razerdp.util.animation.AnimationHelper


class QueryPop : BasePopupWindow, OnItemClickListener {
    ...
    var listener: ((jgcode: String, jyzt: String, lczt: String, sfdb: String, khmc: String) -> Unit)? = null

    constructor(context: Dialog?, listener: ((jgcode: String, jyzt: String, lczt: String, sfdb: String, khmc: String) -> Unit)? = null) : super(context) {
        init(listener)
    }

    constructor(context: Context?,listener: ((jgcode: String, jyzt: String, lczt: String, sfdb: String, khmc: String) -> Unit)? = null) : super(context) {
        init( listener)
    }

    private fun init(listener: ((jgcode: String, jyzt: String, lczt: String, sfdb: String, khmc: String) -> Unit)?) {
        this.bean = bean
        ...
        dataBind?.search?.setOnClickListener {
            isChecked = true
            listener?.invoke(dataBind?.tvValue?.text.toString(), dataBind?.tvCun?.text.toString(), dataBind?.tvZu?.text.toString(), dataBind?.tvShifou?.text.toString(), dataBind?.tvZjhm?.text.toString())
            dismiss()
        }
    ...
    lateinit var dataBind: PopQueryRecyclerBinding
    override fun onCreateContentView(): View { //        dataBind = DataBindingUtil.inflate(LayoutInflater.from(context),R.layout.pop_down_recycler,null,false)
        dataBind = PopQueryRecyclerBinding.bind(createPopupById(R.layout.pop_query_recycler))
    ...
    override fun onDismiss() {
        if (isChecked) listener?.invoke(dataBind?.tvValue?.text.toString(), dataBind?.tvCun?.text.toString(), dataBind?.tvZu?.text.toString(), dataBind?.tvShifou?.text.toString(), dataBind?.tvZjhm?.text.toString())
