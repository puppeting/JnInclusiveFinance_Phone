...
class MyDateUtils {
  ...
  static bool isToday(int milliseconds, {bool isUtc = false, int? locMs}) {
    ...
    DateTime now;
    if (locMs != null) {
      now = MyDateUtils.getDateTimeByMs(locMs);
    } else {
  ...
  static bool isWeek(int ms, {bool isUtc = false, int? locMs}) {
    ...
    DateTime _now;
    if (locMs != null) {
      _now = MyDateUtils.getDateTimeByMs(locMs, isUtc: isUtc);
    } else {
  ...
  static DateTime addMonths(DateTime date, int months) {
    var res = MyDateUtils.copyWith(date, month: date.month + months);
    if (date.day != res.day) res = MyDateUtils.copyWith(res, day: 0);
    return res;
  }
